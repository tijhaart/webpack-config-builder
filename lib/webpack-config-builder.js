...
/***/ function(module, exports, __webpack_require__) {
	...
	function Config(state) {
	  ...
	  function toJs() {
	    try {
	      return _fp2.default.flow(reducers)(state).remove(HOOKS_KEY).toJS();
	    } catch (err) {
	      error(err);
	...
	function useHook(hook, reducer) {
	  return this.use(function (c) {
	    return c.updateIn([HOOKS_KEY, hook], function (reducers) {
	...
	function useEmit(hook) {
	  return this.use(function (c) {
	    var reducers = c.getIn([HOOKS_KEY, hook], false);

	    if (reducers) {
	      c = _fp2.default.flow(reducers)(c);
	    }
